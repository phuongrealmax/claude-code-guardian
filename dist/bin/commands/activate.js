...
async function activateLicense() {
    ...
    try {
        ...
        const licenseKey = await rl.question(chalk.white('Enter your license key (format: CGS-XXXX-XXXX-XXXX): '));
        if (!licenseKey || !licenseKey.startsWith('CGS-')) {
            console.log(chalk.red('\n❌ Invalid license key format'));
            console.log(chalk.gray('License keys should start with CGS- followed by your tier code'));
            rl.close();
            return;
        }
        ...
        if (!verifyResult.valid) {
            ...
            console.log(chalk.gray('If you need help, contact: hello@codeguardian.studio'));
            rl.close();
            return;
        }
...
async function verifyLicenseKey(licenseKey) {
    try {
        ...
        const response = await fetch('https://codeguardian.studio/api/license/verify', {
            method: 'POST',
            headers: {
