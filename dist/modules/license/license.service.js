...
export class LicenseService {
     ...
     */
    getLicenseByStripeSubscription(subscriptionId) {
        const stmt = this.db.prepare(`
    ...
    verifyLicense(request) {
        ...
        if (request.machineId && license.seats) {
            ...
            const isMachineRegistered = activeMachines.some(m => m.machine_id === request.machineId);
     ...
     */
    updateLicenseStatus(licenseKey, status, expiresAt) {
        const now = Date.now();
    ...
    `);
        return stmt.all(licenseId, cutoff);
    }
    /**
     * Register or update a machine
     */
    registerMachine(licenseId, machineId) {
        const id = crypto.randomUUID();
        const now = Date.now();
        const stmt = this.db.prepare(`
     ...
     * Update verification stats
     */
    updateVerificationStats(licenseId) {
        const stmt = this.db.prepare(`
      UPDATE licenses
    ...
    `);
        ...
        const byTier = { dev: 0, team: 0, enterprise: 0 };
    ...
    `);
        ...
        const byStatus = { active: 0, inactive: 0, cancelled: 0, expired: 0 };
